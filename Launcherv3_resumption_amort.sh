...
###############################################################################
#  Launcherv3_resumption_amort.sh
#  Approche rTLS : 1 full → N resumed (amortissement)
#
#  Phase 1 : 1 full handshake → établit la session
#  Phase 2 : 500 resumed handshakes → réutilisent la session
#
#  Usage: ./Launcherv3_resumption_amort.sh [tls|quic] [none|simple|stable|unstable] [loss-percent] [delay-ms]
###############################################################################
...
FULL_RUNS=1
RESUMED_RUNS=500
RUNS_PER_TEST=$((FULL_RUNS + RESUMED_RUNS))
...
echo "=============================================================================="
echo "  rTLS APPROACH : 1 Full → N Resumed (Amortissement)"
echo "=============================================================================="
echo "  Protocol:        $PROTOCOL"
echo "  Runs/test:       $RUNS_PER_TEST (1 full + $RESUMED_RUNS resumed)"
...
for SIG_ALG in "${!PAIRS[@]}"; do
    ...
    # Lancer le client rTLS
    echo "[CLIENT] Starting rTLS amortization test..."
    ...
    docker run --cap-add=NET_ADMIN \
        ...
        $IMAGE ./perftestClientResumptionAmort.sh
...
echo ""
echo "=============================================================================="
echo "  rTLS AMORTIZATION TESTS COMPLETED"
